        ...
        <div class="card-body">
            ...
            <div class="table-responsive">
                <table class="table table-striped">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="{{ $agendamento->statusAgendamento == 'confirmado' ? 'status-confirmado' : ($agendamento->statusAgendamento == 'cancelado' ? 'status-cancelado' : '') }}">{{ ucfirst($agendamento->statusAgendamento) }}</td>
                            <td>
                                @if($agendamento->statusAgendamento == 'pendente' && $currentDate >= $dayBefore && $currentDate < $agendamentoDate)
                                    <select class="select-opcoes" onchange="handleOptionChange(this, {{ $agendamento->idAgendamento }})">
                                        <option value="">Selecione</option>
                                        <option value="confirmar">Confirmar</option>
                                        <option value="cancelar">Cancelar</option>
                                    </select>

                                    <form id="confirmar-form-{{ $agendamento->idAgendamento }}" action="{{ route('agendamentos.confirmarAgendamento', $agendamento->idAgendamento) }}" method="POST" class="hidden-form">
                                        @csrf
                                        @method('PUT')
                                    </form>

                                    <form id="cancelar-form-{{ $agendamento->idAgendamento }}" action="{{ route('agendamentos.cancelarAgendamento', $agendamento->idAgendamento) }}" method="POST" class="hidden-form">
                                        @csrf
                                        @method('PUT')
                                    </form>
                                @endif
                ...
                <div id="descricao-opcoes" class="descricao-opcoes">
                    ...
                    Esse campo ficará disponível para você confirmar ou cancelar seu agendamento um dia antes do mesmo.
                </div>
            </div>
        </div>
    </div>
